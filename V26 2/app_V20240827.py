...
def initialize_app():
    ...
    # Load Pipeline
    path_model = r'models'
    model_filename = r'/26_1_Best_Model.pkl'
    with open(path_model + model_filename, 'rb') as file:
        model = joblib.load(file)
    print('File loades -->' , path_model + model_filename)
    
    # Define columns
    selected_features = ['sex', 'age', 'bmi', 'active_smoking', 'preoperative_hemoglobin_level',
                         ...
                         'anastomotic_technique', 'surgeon_experience',
...
def parser_user_input(dataframe_input , model , selected_features , target , dictionary_categorical_features):
    ...
    for i in dictionary_categorical_features.keys():
        if i not in ['approach' , 'surgeon_experience']:
    ...
    posible_values = list(product(values_of_surgeon_experience , values_of_approach))
    min_probability = {-1 : 2.0}
    max_probability = {-1 : -1.0}
    
    for i in range(len(posible_values)):
        print('#' * 50)
        print('Making prediction usung surgeon experience = ' , posible_values[i][0] , 'and Approach =' , posible_values[i][1])
    ...
    pivot_df = pd.pivot_table(aux_df,
                              values = ['Probability'],
                              index = ['Surgeon Experience' , 'Approach'],
    ...
    df = pd.DataFrame(index=values_of_approach.keys(), columns=values_of_surgeon_experience.keys())
    # Fill the DataFrame with the product of their corresponding values
    for config_key, config_value in values_of_approach.items():
        for exp_key, exp_value in values_of_surgeon_experience.items():
            df.at[config_key, exp_key] = (config_value, exp_value)
    
    for i in range(len(posible_values)):
        aux_approach = posible_values[i][1]
        aux_surgeon = posible_values[i][0]
        df.loc[aux_approach , aux_surgeon] = probabilities[i].values[0]
    
    # Format of prediction
    formatted_df = pivot_df.style.format({"Probability": "{:.6f}".format})
    # Create message to show the best option
    best_option = f"With **Surgeon Experience:** {posible_values[list(min_probability.keys())[-1]][0]}, **Approach:** {posible_values[list(min_probability.keys())[-1]][1]}, the likelihood of anastomotic leakage is the lowest with a value of:{min_probability[list(min_probability.keys())[-1]] * 100 : .6f}%." 
